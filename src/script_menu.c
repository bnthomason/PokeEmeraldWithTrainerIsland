...
static void CreateLilycoveSSTidalMultichoice(void)
{
    ...
    if (CheckBagHasItem(ITEM_TRAINER_TICKET, 1) == TRUE && FlagGet(FLAG_ENABLE_TRAINER_ISLAND) == TRUE)
    {
        if (gSpecialVar_0x8004 == 0)
        {
            sLilycoveSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_TRAINER_ISLAND;
            selectionCount++;
        }

        if (gSpecialVar_0x8004 == 1 && FlagGet(FLAG_SHOWN_TRAINER_TICKET) == FALSE)
        {
            sLilycoveSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_TRAINER_ISLAND;
            selectionCount++;
            FlagSet(FLAG_SHOWN_TRAINER_TICKET);
        }
    }
	
    if (CheckBagHasItem(ITEM_RYUKU_ISLAND_TICKET, 1) == TRUE && FlagGet(FLAG_ENABLE_RYUKU_ISLAND) == TRUE)
    {
        if (gSpecialVar_0x8004 == 0)
        {
            sLilycoveSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_RYUKU_ISLAND;
            selectionCount++;
        }

        if (gSpecialVar_0x8004 == 1 && FlagGet(FLAG_SHOWN_RYUKU_ISLAND_TICKET) == FALSE)
        {
            sLilycoveSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_RYUKU_ISLAND;
            selectionCount++;
            FlagSet(FLAG_SHOWN_RYUKU_ISLAND_TICKET);
        }
    }
