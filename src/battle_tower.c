...
// Comment out the following line if you have changed .iv to go 0-31, instead of 0-255 as in vanilla.
            ...
            if ((partyData[i].nature > 0) || (partyData[i].gender > 0))
                CreateMonWithGenderNatureLetter(&gPlayerParty[i + 3], partyData[i].species, partyData[i].lvl, fixedIV, partyData[i].nature, 0, partyData[i].shiny ? OT_ID_SHINY : OT_ID_RANDOM_NO_SHINY);
            else
            {
